...
type Step = {
  id: number; stepNumber: number; title: string; description: string;
  iconName?: string | null; displayOrder: number; visible: boolean;
};
...
function StepRow({ step, onSave, onDelete }: { step: Step; onSave: (id: number, data: Partial<Step>) => void; onDelete: (id: number) => void }) {
      ...
      <div className="bg-white border-2 border-[#173f35] rounded-2xl p-5 shadow-md">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="w-full border border-[#e5ded3] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#173f35] resize-none"
              rows={3}
  ...
  return (
    <div className={`bg-white border rounded-2xl p-4 flex items-start gap-4 group transition-all hover:shadow-md ${!step.visible ? "opacity-60" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          {step.iconName && <span className="text-[10px] bg-[#f7f4ef] text-[#65736f] px-2 py-0.5 rounded-full">{step.iconName}</span>}
          <span className="text-[10px] text-[#8a958f]">Order: {step.displayOrder}</span>
        </div>
      </div>
      <div className="flex items-center gap-1 shrink-0">
        <button
          onClick={() => onSave(step.id, { visible: !step.visible })}
          className={`p-2 rounded-lg transition-colors ${step.visible ? "text-[#173f35] hover:bg-[#173f35]/10" : "text-[#8a958f] hover:bg-[#f7f4ef]"}`}
          title={step.visible ? "Hide step" : "Show step"}
        >
          {step.visible ? <Eye size={16} /> : <EyeOff size={16} />}
        </button>
        <button onClick={() => setEditing(true)} className="p-2 rounded-lg text-[#65736f] hover:bg-[#efe3cf] hover:text-[#10231f] transition-colors">
          <Pencil size={16} />
        </button>
        <button onClick={() => onDelete(step.id)} className="p-2 rounded-lg text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors">
          <Trash2 size={16} />
        </button>
      </div>
    </div>
...
const BLANK_STEP = { stepNumber: 1, title: "", description: "", iconName: "", displayOrder: 0, visible: true };

export default function MethodologyManager() {
  const qc = useQueryClient();
  const { data, isLoading } = useListMethodologySteps();
  const createMutation = useCreateMethodologyStep();
  const updateMutation = useUpdateMethodologyStep();
  const deleteMutation = useDeleteMethodologyStep();
  const [adding, setAdding] = useState(false);
  const [newForm, setNewForm] = useState({ ...BLANK_STEP });

  const invalidate = () => qc.invalidateQueries({ queryKey: ["/api/content/methodology"] });
  ...
  const handleDelete = (id: number) => {
    if (!confirm("Delete this delivery step? It will be removed from the homepage.")) return;
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({
      ...
      onSuccess: () => { invalidate(); setAdding(false); setNewForm({ ...BLANK_STEP }); }
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#65736f] mt-0.5">The 6-step delivery model shown on the homepage. Edit titles, descriptions, or reorder steps.</p>
        </div>
      ...
      <div className="bg-[#efe3cf]/40 border border-[#e5ded3] rounded-2xl p-4 text-sm text-[#65736f]">
        <strong className="text-[#10231f]">How this works:</strong> These steps appear in the "Delivery Model" section on the homepage. Each step has a number, title, and description. Toggle visibility to hide steps temporarily. Changes take effect immediately.
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {steps.map(step => <StepRow key={step.id} step={step as Step} onSave={handleSave} onDelete={handleDelete} />)}
        </div>
      )}
      ...
      {adding && (
        <div className="bg-white border-2 border-[#c59b4a] rounded-2xl p-5 shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-[#10231f] mb-1 uppercase tracking-wide">Description *</label>
              <textarea className="w-full border border-[#e5ded3] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#173f35] resize-none" rows={3} value={newForm.description} onChange={e => setNewForm(f => ({ ...f, description: e.target.value }))} placeholder="What happens in this step?" />
            </div>
            <div>
              <label className="block text-xs font-bold text-[#10231f] mb-1 uppercase tracking-wide">Display Order</label>
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => { setAdding(false); setNewForm({ ...BLANK_STEP }); }} className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-[#e5ded3] text-[#65736f] text-sm hover:bg-[#f7f4ef] transition-colors"><X size={14} /> Cancel</button>
            <button onClick={handleCreate} disabled={!newForm.title || !newForm.description} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#c59b4a] text-[#10231f] text-sm font-bold hover:bg-[#b5893a] transition-colors disabled:opacity-50"><Check size={14} /> Add Step</button>
          </div>
        </div>
      )}

      {steps.length === 0 && !isLoading && !adding && (
        <div className="text-center py-16 text-[#8a958f]">
          <Target className="h-10 w-10 mx-auto mb-3 opacity-30" />
          <p className="font-semibold">No steps yet</p>
          <p className="text-sm mt-1">Click "Add Step" to build your delivery methodology.</p>
        </div>
      )}
    </div>
  );
