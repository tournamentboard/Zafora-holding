...
import { Activity, Trash2, RefreshCw, Loader2, AlertCircle, Filter } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#65736f] mt-0.5">A record of every action taken on your website — edits, new inquiries, project changes, and more.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={fetchLogs}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#e5ded3] text-sm text-[#65736f] bg-white hover:border-[#173f35] transition-colors"
          >
      ...
      <div className="bg-white border border-[#e5ded3] rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-[#8a958f] gap-2">
            ...
            <p className="text-sm">{logs.length === 0 ? "No activity logged yet. Actions you take in this panel will appear here." : "No logs match this filter."}</p>
          </div>
        ) : (
            ...
            {filtered.map((log) => (
              <div key={log.id} className="flex items-start gap-4 px-5 py-3.5 hover:bg-[#faf8f5] transition-colors">
                <div className="shrink-0 mt-0.5 flex flex-col gap-1.5 items-center">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${ACTION_COLORS[log.action] ?? "bg-gray-100 text-gray-600"}`}>
                    {log.action}
                  </span>
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${CATEGORY_COLORS[log.category] ?? "bg-gray-100 text-gray-600"}`}>
                    {log.category}
                  </span>
                </div>
                ...
                <div className="text-xs text-[#8a958f] shrink-0 pt-0.5 whitespace-nowrap">
      ...
      <div className="bg-[#173f35]/5 border border-[#173f35]/15 rounded-2xl p-4 text-sm text-[#65736f] flex gap-3">
        ...
        <span><strong className="text-[#10231f]">What gets logged:</strong> New inquiries, inquiry status changes, project creates/updates/deletes, and partner interest expressions. Settings changes will be added over time.</span>
      </div>
    </div>
  );
}
