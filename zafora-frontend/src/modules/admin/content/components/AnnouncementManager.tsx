...
export default function AnnouncementManager() {
  ...
  useEffect(() => {
    if (data?.value) {
      try { setBar({ ...DEFAULTS, ...JSON.parse(data.value) }); } catch {}
    }
  }, [data?.value]);

  const set = (key: string, val: unknown) => setBar((prev) => ({ ...prev, [key]: val }));

  const handleSave = () => {
    updateMutation.mutate(
      { key: "announcement_bar", data: { value: JSON.stringify(bar) } },
      {
        onSuccess: () => { toast.success("Announcement bar saved"); setSaved(true); setTimeout(() => setSaved(false), 3000); },
      ...
      {bar.message && (
        <div className="rounded-xl overflow-hidden border border-[#e5ded3]">
          ...
          >
            ...
            {bar.link && bar.linkText && <span className="underline opacity-80 text-xs">{bar.linkText}</span>}
          ...
          <div className="bg-white rounded-2xl border border-[#e5ded3] p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="We're now accepting new project inquiries..."
