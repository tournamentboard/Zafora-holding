...
export default function LeadsTable() {
  ...
  const filtered =
    filterStatus === "all" ? data?.leads : data?.leads?.filter((l) => l.status === filterStatus);

  if (isLoading) {
    return (
      <div className="space-y-3 animate-pulse">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-24 bg-white rounded-2xl border border-[#e5ded3]" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="text-[#65736f]">
          Everyone who has contacted you through the website. Click on a person to read their message.
        </p>
      </div>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${
            filterStatus === "all"
              ...
              : "bg-white border border-[#e5ded3] text-[#65736f] hover:border-[#173f35]"
          }`}
        >
          All ({data?.total ?? 0})
        </button>
      ...
      <div className="space-y-3">
        {filtered?.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl border border-[#e5ded3]">
            ...
            <p className="text-[#8a958f] text-sm mt-1">
              When someone contacts you through the website, they will appear here.
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-[#e5ded3]">
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-[#f7f4ef] text-[#65736f]"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#f7f4ef] rounded-xl p-4">
                  ...
                  <div className="font-semibold text-[#10231f]">
                    {REQUEST_LABELS[selectedLead.requestType] ?? selectedLead.requestType}
                  </div>
                </div>
                ...
                {selectedLead.budgetFundingNeed && (
                  <div className="bg-[#f7f4ef] rounded-xl p-4">
                    <div className="text-xs font-semibold text-[#8a958f] uppercase mb-1">Budget / Funding</div>
