...
import {
  Plus, Loader2, Trash2, ChevronRight, ArrowLeft, Eye, EyeOff, HelpCircle, Check,
} from "lucide-react";
import { useFaqs, useCreateFaq, useUpdateFaq, useDeleteFaq, type Faq } from "../services/faqs.service";
...
type FaqForm = Omit<Faq, "id" | "createdAt" | "updatedAt"> & { id?: number };

const EMPTY: FaqForm = {
  question: "", answer: "", category: "general", page: "general", displayOrder: 0, visible: true,
};
...
export default function FaqManager() {
  ...
  const [selected, setSelected] = useState<FaqForm | null>(null);
  const [isNew, setIsNew] = useState(false);
  const [saved, setSaved] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [filterPage, setFilterPage] = useState("all");

  const setField = (key: keyof FaqForm, val: unknown) =>
  ...
  const handleSave = async () => {
    ...
    try {
      if (isNew) {
        ...
        await updateMutation.mutateAsync({ id: selected.id!, data: selected });
  ...
  const toggleVisible = async (faq: Faq) => {
    try {
      await updateMutation.mutateAsync({ id: faq.id, data: { visible: !faq.visible } });
    } catch {
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3 mb-1">
          <button
            onClick={() => { setSelected(null); setIsNew(false); setConfirmDelete(false); }}
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={handleSave}
            disabled={isSaving || !selected.question || !selected.answer}
            ...
            <div className="bg-white rounded-2xl border border-[#e5ded3] p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full border border-[#e5ded3] rounded-xl px-3 py-2.5 text-sm text-[#10231f] focus:outline-none focus:ring-2 focus:ring-[#173f35]/30 bg-white appearance-none capitalize"
                >
                  {PAGES.map((p) => (
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setSelected({ ...EMPTY, displayOrder: faqs.length }); setIsNew(true); }}
        ...
        {["all", ...PAGES].map((p) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold capitalize transition-all ${
              filterPage === p ? "bg-[#173f35] text-white" : "bg-white border border-[#e5ded3] text-[#65736f] hover:bg-[#f7f4ef]"
            }`}
          >
            {p === "all" ? "All pages" : p}
          </button>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-[#e5ded3] py-16 text-center">
          ...
          <button
            ...
            className="mt-3 text-sm text-[#173f35] font-semibold hover:underline"
          >
        ...
        <div className="space-y-3">
          {[...filtered]
            ...
            .map((faq) => (
              <div
                key={faq.id}
                className="bg-white rounded-2xl border border-[#e5ded3] p-5 flex gap-4 items-start hover:shadow-sm transition-all"
              >
                <div
                  className="flex-1 min-w-0 cursor-pointer"
                  onClick={() => { setSelected({ ...faq }); setIsNew(false); }}
                >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-[#f7f4ef] transition-colors text-[#8a958f]"
