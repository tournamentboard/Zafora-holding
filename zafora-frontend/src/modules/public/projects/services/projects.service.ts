...
import type { Project, ProjectInterest } from "@/src/lib/types";
...
async function fetchProjectInterests(projectId: number): Promise<ListInterestsResponse> {
  const res = await apiAxios.get<ListInterestsResponse>(API.PROJECTS.INTERESTS(projectId));
  return res.data;
}
