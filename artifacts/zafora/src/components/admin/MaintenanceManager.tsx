...
export default function MaintenanceManager() {
  ...
  const set = (key: string, val: any) => setMode(prev => ({ ...prev, [key]: val }));
  ...
  const handleSave = () => {
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          qc.invalidateQueries({ queryKey: ["/api/content/settings/maintenance_mode"] });
          setSaved(true);
      ...
      {mode.enabled && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 mt-0.5">Visitors will see the maintenance page instead of your website. The admin panel remains accessible.</p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl border border-[#e5ded3] p-6 space-y-5">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-[#f0ebe3]">
              <div>
                ...
                <p className="text-[11px] text-[#8a958f] mt-0.5">Displays your footer contact email on the maintenance page.</p>
              </div>
              <button
          ...
          <div className="bg-white rounded-2xl border border-[#e5ded3] p-5 space-y-4">
            ...
            <div className="bg-[#f7f4ef] rounded-xl px-4 py-3">
              <p className="text-[11px] text-[#65736f] leading-relaxed">
                When active, all public pages redirect to the maintenance screen. You can still access the admin panel at <strong>/admin</strong>.
              </p>
            </div>
          </div>
        </div>
