...
const PAGES_CONFIG: Record<string, { label: string; sections: { key: string; label: string; desc: string }[] }> = {
  home: {
    ...
    sections: [
      ...
      { key: "stats", label: "Stats Bar", desc: "Key numbers ($2.4B+, 12+ countries, etc.)" },
  ...
  services: {
    label: "Services",
    sections: [
      { key: "hero", label: "Hero", desc: "Services page header with mosaic images" },
  ...
  submit: {
    ...
    sections: [
      ...
      { key: "form", label: "Submission Form", desc: "3-step numbered contact form" },
...
export default function SectionVisibilityManager() {
  ...
  const handleSave = () => {
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          qc.invalidateQueries({ queryKey: ["/api/content/settings/section_visibility"] });
          setSaved(true);
    ...
    <div className="space-y-5">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                activePage === key ? "bg-[#173f35] text-white shadow-md" : "bg-white border border-[#e5ded3] text-[#65736f] hover:bg-[#f7f4ef]"
              }`}
            >
              {label}
              {hidden > 0 && (
      ...
      <div className="bg-white rounded-2xl border border-[#e5ded3] overflow-hidden">
        ...
        <div className="divide-y divide-[#f0ebe3]">
          {currentPage.sections.map(section => {
            ...
            return (
              <div key={section.key} className={`flex items-center justify-between px-6 py-4 transition-colors ${!isVisible ? "bg-[#f7f4ef]/60" : ""}`}>
      ...
      <div className="bg-[#f7f4ef] border border-[#e5ded3] rounded-xl px-5 py-3 flex items-center gap-3">
        ...
        <p className="text-xs text-[#65736f]">
          Hidden sections are removed from the live page immediately after saving. This does not delete the content — you can re-enable sections at any time.
        </p>
      </div>
    </div>
  );
